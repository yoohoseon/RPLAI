  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      ...
      if (content.includes('bg-[#3182F6]')) {
        content = content.replace(/bg-\[#3182F6\] text-white hover:bg-\[#1B64DA\]/g, 'bg-[#F2F4F6] text-[#333D4B] hover:bg-[#E5E8EB] active:bg-[#D1D6DB]');
        ...
        content = content.replace(/text-white/g, 'text-[#333D4B]'); // Be careful with this, but since it's button specific usually it's fine. Wait, text-white might be used elsewhere.
        changed = true;
      }

      // Instead of replacing text-white blindly, let's fix the exact button classes
      content = content.replace(/bg-\[#F2F4F6\] text-white/g, 'bg-[#F2F4F6] text-[#333D4B]');
      ...
      // Update specific text colors to #333D4B
