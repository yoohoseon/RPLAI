import { fetchBrandDatas, fetchTeams } from '@/app/lib/data';
import {
    Table,
    TableBody,
    TableCell,
...
import { DaHistoryTableRow } from '@/components/history/da-history-table-row';
import HistoryFilters from '@/components/history/history-filters';
import PaginationControls from '@/components/ui/pagination-controls';

interface DaHistoryPageProps {
...
export default async function DaHistoryPage({ searchParams }: DaHistoryPageProps) {
    const params = await searchParams;
    const currentPage = Number(params.page) || 1;
    const limit = 10;

    const { data: analyses, totalPages, totalCount } = await fetchBrandDatas(
        {
            brand: params.brand,
            ...
            <div className="w-full py-12 px-6 space-y-10 animate-in fade-in duration-700">
                <div className="flex items-end justify-between border-none pb-8">
                    <div className="space-y-2">
                        <h1 className="text-3xl font-bold tracking-tight text-[#333333]">디지털 에셋 분석 이력</h1>
                        <p className="text-[17px] font-medium text-[#4E5968]">과거의 DA 기반 타겟 및 커뮤니케이션 전략 설정 기록을 확인하세요.</p>
                    </div>
                    <Link href="/main/da">
                            ...
                            <TableBody>
                                {analyses.length === 0 ? (
                                    ...
                                    analyses.map((analysis: any) => (
                                        <DaHistoryTableRow key={analysis.id} analysis={analysis} />
                                    ))
                                )}
                            </TableBody>
                        </Table>
